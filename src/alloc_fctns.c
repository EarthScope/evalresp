...
struct evresp_complex *alloc_complex(int npts) {
    struct evresp_complex *cptr;

    if (npts) {
        if ((cptr = (struct evresp_complex *) malloc(npts * sizeof(struct evresp_complex)))
                == (struct evresp_complex *) NULL) {
            error_exit(OUT_OF_MEMORY,
                    "alloc_complex; malloc() failed for (complex) vector");
        }
    } else
        cptr = (struct evresp_complex *) NULL;
...
struct blkt *alloc_pz(void) {
    ...
    blkt_ptr->blkt_info.pole_zero.zeros = (struct evresp_complex *) NULL;
    blkt_ptr->blkt_info.pole_zero.poles = (struct evresp_complex *) NULL;
...
void free_pz(struct blkt *blkt_ptr) {
    if (blkt_ptr != (struct blkt *) NULL) {
        if (blkt_ptr->blkt_info.pole_zero.zeros != (struct evresp_complex *) NULL)
            free(blkt_ptr->blkt_info.pole_zero.zeros);
        if (blkt_ptr->blkt_info.pole_zero.poles != (struct evresp_complex *) NULL)
