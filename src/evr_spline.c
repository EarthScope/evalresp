    ...
    *p_num_retvals = 0; /* initialize count */
    ...
    for (i = 0; i < num_xvals; ++i) { /* for each value in "new" abscissa array */
        ...
        if ((x - t[0]) * (x - t[used]) <= 0) { /* calculate value, enter into array, inc index */
            (*p_retvals_arr)[(*p_num_retvals)++] = interpolate(used, t, y, z, x,
                    tension, false);
        } else
            range_count++;
    }
...
double quotient_sinh_func(double x, double y) {
    return ((x * x * x - x)
            + (x * x * x * x * x / 20.0 - x * x * x / 6.0 + 7.0 * x / 60.0)
                    * (y * y)
            + (x * x * x * x * x * x * x / 840.0 - x * x * x * x * x / 120.0
                    + 7.0 * x * x * x / 360.0 - 31.0 * x / 2520.0)
                    * (y * y * y * y));
}
