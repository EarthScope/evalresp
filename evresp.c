...
int evresp_(char *sta, char *cha, char *net, char *locid, char *datime, 
	    ...
	    int ldatime, int lunits, int lfile, int lrtype, int lverbose, int useTotalSensitivityFlag, double x_for_b62)
{
  ...
  first = evresp(sta, cha, net, locid, datime, units, file, dfreqs, nfreqs,
             rtype, verbose, start, stop, flag, useTotalSensitivityFlag, x_for_b62);
...
char SEEDUNITS[][UNITS_STR_LEN] = {"Undef Units", "Displacement", "Velocity",
                        "Acceleration", "Counts", "Volts", "", "Pascals", "Tesla", "Centigrade"};
...
struct response *evresp_itp(char *stalst, char *chalst, char *net_code,
                            ...
                            double listinterp_tension, int useTotalSensitivityFlag, double x_for_b62)
{
				...
				if(!(err_type = setjmp(jump_buffer))) 
				{
					...
					if(new_file && which_matched >= 0) 
					{
						        ...
						        && this_channel.first_stage->first_blkt->type == LIST)
						{
							...
							memcpy (freqs, 
							  this_channel.first_stage->first_blkt->blkt_info.list.freq, sizeof(double) * nfreqs); /*cp*/
						...
						calc_resp(&this_channel, freqs, nfreqs, output, units,
						     start_stage, stop_stage, useTotalSensitivityFlag, x_for_b62);
					...
					if(which_matched >= 0) 
					{
						...
						if (strncmp (this_channel.first_units, "C -", 3) == 0)  
						{
							if (strcmp(units, "VEL") != 0)  
							{
								if(strcmp(units, "DEF") != 0)  
								{
									fprintf(stderr, "%s WARNING: OUTPUT %s does not make sense if INPUT is TEMPERATURE\n",
									                    myLabel, units);
									strcpy (units, "VEL");
									fprintf(stderr, "%s      OUTPUT units are reset and interpreted as  DEGREES CENTIGRADE\n", myLabel);
								}
							}
						}
						     ...
						     && this_channel.first_stage->first_blkt->type == LIST) 
						{
							...
							nfreqs = this_channel.first_stage->first_blkt->blkt_info.list.nresp;
							freqs = (double *) malloc(sizeof(double) * nfreqs); /* malloc a new vector */
    						memcpy (freqs, this_channel.first_stage->first_blkt->blkt_info.list.freq, 
							     sizeof(double) * nfreqs); /*cp*/
							resp->rvec = alloc_complex(nfreqs);	
							output=resp->rvec;
							resp->nfreqs = nfreqs;
							resp->freqs = (double *) malloc(sizeof(double) * nfreqs); /* malloc a new vector */
    						memcpy (resp->freqs, this_channel.first_stage->first_blkt->blkt_info.list.freq,
							       sizeof(double) * nfreqs); /*cp*/
						}
						else	
						{
...
struct response *evresp(char *stalst, char *chalst, char *net_code,
                        ...
                        char *file, double *freqs, int nfreqs,
                        char *rtype, char *verbose, int start_stage,
                        int stop_stage, int stdio_flag, int useTotalSensitivityFlag, double x_for_b62)
{
